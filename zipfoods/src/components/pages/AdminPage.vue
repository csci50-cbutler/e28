<template>
  <div>
    ...
    <em>
      Disclaimer: This functionality is for demonstration purposes only. In a
      real-world application, this functionality must be locked down to
      administrators only, and seeding is not typically done via a client-side
      interface.
    </em>
    ...
    seedApi: function() {
      for (let collectionName in this.collections) {
        for (let key in this.collections[collectionName]) {
          app.api.add(collectionName, this.collections[collectionName][key]);
        }
      }
